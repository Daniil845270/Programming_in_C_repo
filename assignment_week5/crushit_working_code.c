...
void test(void)
{
    ...
    // assert(invalid_init_options(&s_test, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") == false);
    // assert(invalid_init_options(&s_test, "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA") == true);
    // assert(invalid_init_options(&s_test, "ABCDBCDAACDAABDAABCAABCDABCD.A") == false);
    // assert(invalid_init_options(&s_test, ".....") == false);
    // assert(invalid_init_options(&s_test, "abcde") == true);
    // assert(invalid_init_options(&s_test, "\0\0\0\0\0") == true);
    // assert(invalid_init_options(&s_test, "\0\0\0\0") == true);
    // assert(invalid_init_options(&s_test, "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ") == false);
    // assert(invalid_init_options(&s_test, "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ") == true);
    // char test_alphabet_string[30] = {0};
    // init_uprcase_alphabet(test_alphabet_string);
    // assert(test_alphabet_string[0] == 'A');
    // assert(test_alphabet_string[25] == 'Z');
    // //printf("was here\n");
    // assert(test_alphabet_string[26] == '\0');


    // initialise_board(&s_test, "AAAAA");
    // assert(s_test.rows == 1);
    // initialise_board(&s_test, "AAAAAAAAAA");
    // assert(s_test.rows == 2);
    // initialise_board(&s_test, "AAAAAAAAAAAAAAAAAAAA");
    // assert(s_test.rows == 4);

    // initialise_board(&s_test, "ABCDEFGHIJKLMNOPQRSTUVWXY");
    // //two_d_arr_printer(&s_test);

    // assert(initialise(&s_test, "ABCDEFGHIJKLMNOPQRSTUVWXY") == true);
    // char str[WIDTH*MAXROWS+1];
    // assert(tostring(&s_test, str) == true);
    // assert(strcmp(str,    "ABCDEFGHIJKLMNOPQRSTUVWXY") == 0);

    // assert(initialise(&s_test, "ABCDEABCDEFGHIJKLMNOPQRSTUVWXY") == true);
    // assert(tostring(&s_test, str) == true);
    // assert(strcmp(str,    "ABCDEABCDEFGHIJKLMNOPQRSTUVWXY") == 0);

    // assert(initialise(&s_test, "ABCDEABCDEABCDEABCDEABCDEFGHIJKLMNOPQRSTUVWXY") == true);
    // assert(tostring(&s_test, str) == true);
    // assert(strcmp(str,    "ABCDEABCDEFGHIJKLMNOPQRSTUVWXY") == 0);
...
bool initialise(state* s, const char* str)
{
    ...
    char filename_or_string[(WIDTH*MAXROWS+1)] = {'0'};
...
//were we explained how to use fgets to read multiple files?
void tryfile(const char* str, char* filename_or_string)
{
    ...
    //try add NULL at strlen(str) if there is an overflow somehow
     
    FILE* filepointer = fopen(filename_or_string, "r");
    if (filepointer != NULL){
        char temp_str[(WIDTH*MAXROWS+1)] = {'0'};
        int fillup_cnt = 0;
        while (fscanf(filepointer, "%s", temp_str) == 1){
            for (long unsigned int letter = 0; letter < strlen(temp_str); letter++){
                filename_or_string[fillup_cnt] = temp_str[letter];
                fillup_cnt++;
            }
        }
        fclose(filepointer);
...
void two_d_arr_printer(state* s)
{
    for (int row = 0; row < s->rows; row++){
        for (int column = 0; column < WIDTH; column++){
...
bool tostring(const state* s, char* str) 
{
    ...
    two_d_arr_printer(s);
    printf("tostring result case1 %s\n", str);
    return true;
}
    ...
    for (int row = 0; row < HEIGHT; row++){
        ...
        for (int column = 0; column < WIDTH; column++){
            ...
            if (pattern_rec->board[row][column] == 'R'){
                s->board[(row + indentaion)][column] = '.'; //crushit.c:312:25: runtime error: index -1 out of bounds for type 'char [20][5]'
                *any_mathces = true;
            }
        }
    }
...
{
    for (int row = 0; row < pattern_rec->rows; row++){
...
{
    for (int row = 1; row < pattern_rec->rows - 1; row++){
        for (int column = 0; column < WIDTH; column++){
